...
static int my_probe(struct platform_device *pdev){
	...
	//Second method to get Linux IRQ number
	irq = platform_get_irq(pdev,0);
	if(irq<0){
		dev_err(dev, "irq isnot available\n");
		return -EINVAL;
	}
	...
	dev_info(dev, "my_probe function has exited.\n");
...
static int __exit my_remove(struct platform_device *pdev){
	...
	dev_info(&pdev->dev,"my_remove() function has exited. \n");
	return 0;
}
