...
static irqreturn_t KEY_ISR1(int irq, void *data)
{
	...
	dev_info(&(pot_data->spi->dev), "interrupt MIKROBUS_KEY1 received. key: %s\n",
			 HELLO_KEYS_NAME1);
...
{
	struct mcp4131_data *pot_data = data;
	dev_info(&(pot_data->spi->dev), "interrupt MIKROBUS_KEY2 received. key: %s\n",
			 HELLO_KEYS_NAME2);
...
static int mcp4131_probe(struct spi_device *spi)
{
	...
	device_for_each_child_node(dev, child){
		...
		if (strcmp(label_name,"MIKROBUS_KEY_1") == 0) {
