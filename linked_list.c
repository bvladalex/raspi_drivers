...
static ssize_t my_dev_write(struct file *file, const char __user *buf, size_t size, loff_t *offset)
{
	...
	printk("Element actual size is: %d\n", size);
	printk("Remaining string is: %s\n", buf);
	...
	printk("RSize to read becomes: %d\n", size_to_read);
